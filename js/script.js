...
var navSections = ['about', 'experience', 'projects', 'education', 'contact'];
...
function updateActiveNav() {
  ...
  if (atBottom) {
    ...
    for (var i = navSections.length - 1; i >= 0; i--) {
      ...
      if (sec && sec.offsetTop <= scrollY) { active = navSections[i]; break; }
    }
  }
  navItems.forEach(function(item) {
    item.style.color = item.getAttribute('href') === '#' + active ? 'var(--text)' : '';
  });
}
...
window.addEventListener('scroll', function() {
  if (!navScrollTicking) {
    requestAnimationFrame(function() { updateActiveNav(); navScrollTicking = false; });
...
(function initHeroParticles() {
  ...
  if (!container) return;
  // Skip particle creation entirely on touch devices
...
// =============================================
// LAZY VIDEO — load only when visible
// =============================================
(function() {
